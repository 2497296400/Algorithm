...
//        }
    ...
    private static void quickSort(int[] arr, int left, int right) {
        if (left < right) {
            ...
            quickSort(arr, left, privot[0]-1);
            quickSort(arr, privot[1] + 1, right);
        }
    }
    ...
    private static int[] getPrivot(int[] arr, int left, int right) {
        int curMin = left - 1;
        int curMax = right;
        int curIndexArr = arr[right];
        while (left < curMax) {
            if (arr[left] < curIndexArr) {
                swap(arr, ++curMin, left++);
            } else if (arr[left] > curIndexArr) {
                swap(arr, --curMax, left);
            } else {
                left++;
            }
        }
        swap(arr, right, curMax);
